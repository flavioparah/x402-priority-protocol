...
**The broker is trusted to be neutral.** The cross-operator Trust-Score Broker (described in [`x402-trust-score` §2](./rfc/x402-trust-score.md#2-architecture)) is a single point of trust for cross-operator reputation. The broker implementation exists as a separate local service in this repository (`broker/` subdirectory) and is exercised by the local broker test suite. A public neutral broker deployment and federation are still pending. Until then, cross-operator scoring degrades to single-operator/local signals when the Shield isn't pointed at a running broker. Broker neutrality is governance-only (see [`BROKER-GOVERNANCE.md`](../BROKER-GOVERNANCE.md)) — there is no cryptographic enforcement. The roadmap to mitigate this is signed audit log + change-window governance (Phase 4 of the cross-op roadmap); `GET /audit/:date` exists in the broker scaffold (in-memory) but signed daily audit bundles and multi-broker federation are not implemented yet.
...
| **RFC 8785 JSON Canonicalization** | [`broker/lib/canonical-json.js`](../broker/lib/canonical-json.js) | Implemented in the broker scaffold for `provider_signature` over `/attest` and `/report` bodies. Minimal implementation sufficient for flat broker bodies; does not handle NFC normalization or non-BMP surrogate pairs (caller-side responsibility). The Shield itself does not need RFC 8785 today — applies only at the broker boundary. |
...
| 7 | **`negative balance` refund race is a future/spec-level concern.** The current reference implementation does not expose a general refund endpoint. Concurrent refund + signed retry semantics ([`x402-priority` §12](./rfc/x402-priority.md#12-open-issues)) become relevant only once refunds ship. | Cosmetic / accounting only today; no money loss because refunds are not yet implemented. | Spec-level — deferred until refund flow exists. |
| 8 | **x402 ecosystem drift.** External x402 implementations are evolving quickly across Solana and other chains (QuickNode, Coinbase x402 reference, others). Header names, facilitator flows, token choices, and payment payload conventions may diverge from this Shield's current `Authorization: x402 ...` convention. | Cross-implementation interoperability may break as the ecosystem standardizes. Clients written against one implementation may need adapters for others. | Keep the wire-level RFC ([`x402-priority`](./rfc/x402-priority.md)) explicit, version headers (`X-x402-Spec-Version`), and run compatibility tests against major ecosystem clients as they emerge. Track the x402 working group output if/when one forms. |
