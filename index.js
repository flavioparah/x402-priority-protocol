...
// API home: humanized landing for the bare / route so visitors hitting
// api.rpcpriority.com / mainnet.rpcpriority.com / etc. directly in a browser
// see a structured page instead of the marketing landing or 404. Registered
// BEFORE express.static so it overrides any public/index.html in the image.
app.get("/", (req, res) => {
  const upstream = CONFIG.REAL_RPC_URL;
  const network = upstream.includes("mainnet") ? "mainnet"
    : upstream.includes("devnet") ? "devnet"
    : "unknown";
  const isDemo = !!CONFIG.TRUST_DEPOSITS;
  const badgeClass = network === "mainnet" ? "mainnet" : isDemo ? "demo" : "devnet";
  const badgeText = network === "mainnet" ? "MAINNET" : isDemo ? "DEMO" : "DEVNET";
  const networkLabel = network === "mainnet" ? "Solana Mainnet"
    : isDemo ? "Trust-Score Demo (devnet)"
    : "Solana Devnet";
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>x402 Priority Gateway — ${networkLabel}</title>
  <link rel="icon" type="image/x-icon" href="https://rpcpriority.com/favicon.ico">
  <meta name="theme-color" content="#14F195">
  <link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;700&family=Inter:wght@300;400;600;800&display=swap" rel="stylesheet">
  <style>
    body{font-family:'Inter',system-ui;background:#0a0a0a;color:#ededed;margin:0}
    .wrap{max-width:780px;margin:60px auto;padding:0 22px;line-height:1.65}
    h1{color:#14F195;font-weight:800;letter-spacing:-0.5px;margin:0;font-size:28px}
    h2{color:#9945FF;font-weight:600;margin:32px 0 12px;font-size:17px}
    .mono,code{font-family:'JetBrains Mono',monospace}
    code{background:#1a1a1a;padding:2px 7px;border-radius:4px;font-size:13px}
    a{color:#9945FF;text-decoration:none;border-bottom:1px dotted #9945FF44}
    a:hover{border-bottom-color:#9945FF}
    pre{background:#1a1a1a;padding:14px 18px;border-radius:8px;overflow-x:auto;font-size:13px;line-height:1.5;border:1px solid rgba(255,255,255,0.08)}
    .badge{display:inline-block;padding:3px 10px;border-radius:4px;font-size:11px;font-weight:700;letter-spacing:0.6px;font-family:'JetBrains Mono',monospace}
    .mainnet{background:#14F195;color:#000}
    .devnet{background:#f9a826;color:#000}
    .demo{background:#9945FF;color:#fff}
    table{width:100%;border-collapse:collapse;margin:8px 0}
    th,td{text-align:left;padding:8px 12px;border-bottom:1px solid rgba(255,255,255,0.06);font-size:14px;vertical-align:top}
    th{opacity:0.5;font-weight:600;text-transform:uppercase;font-size:11px;letter-spacing:0.5px;width:30%}
    ul{padding-left:20px;margin:8px 0}
    li{margin:6px 0}
    .row{display:flex;align-items:center;gap:14px;flex-wrap:wrap;margin-bottom:14px}
    .footer{margin-top:50px;padding-top:24px;border-top:1px solid rgba(255,255,255,0.08);opacity:0.65;font-size:13px}
    p.lead{opacity:0.85}
  </style>
</head>
<body>
<div class="wrap">
  <div class="row">
    ...
    <h1>x402 Priority Gateway</h1>
  </div>
  <p class="lead">HTTP 402 priority pricing for Solana RPC. Per-request payment in SOL via the <a href="https://x402.org" target="_blank">x402 protocol</a>. Cryptographic identity (Ed25519), Trust-Score reputation, no API key required.</p>

  <h2>Gateway info</h2>
  <table>
    <tr><th>Network</th><td class="mono">${network}</td></tr>
    <tr><th>Operator</th><td class="mono">${CONFIG.PAYMENT_DESTINATION}</td></tr>
    <tr><th>Upstream RPC</th><td class="mono">${upstream}</td></tr>
    <tr><th>Base price</th><td class="mono">${CONFIG.BASE_PRICE_MICRO_LAMPORTS.toLocaleString()} µL (${CONFIG.BASE_PRICE_MICRO_LAMPORTS/1000} lamports)</td></tr>
    <tr><th>Max price</th><td class="mono">${CONFIG.MAX_PRICE_MICRO_LAMPORTS.toLocaleString()} µL (${CONFIG.MAX_PRICE_MICRO_LAMPORTS/1000} lamports)</td></tr>
    <tr><th>Load threshold</th><td class="mono">${CONFIG.RPC_LOAD_THRESHOLD}</td></tr>
    <tr><th>Nonce TTL</th><td class="mono">${CONFIG.NONCE_TTL_MS / 1000}s</td></tr>
    <tr><th>Trusted deposits</th><td class="mono">${isDemo ? "ON (demo only — no on-chain verification)" : "OFF (production — on-chain verified)"}</td></tr>
  </table>

  <h2>Quick start</h2>
  <pre>curl -X POST ${proto}://${host}/rpc \\
  -H "Content-Type: application/json" \\
  -d '{"jsonrpc":"2.0","id":1,"method":"getHealth","params":[]}'</pre>
  <p style="font-size:13px;opacity:0.7">Under load this returns <code>HTTP 402</code> with a signed nonce challenge. Sign it with Ed25519 and retry with <code>Authorization: x402 &lt;sig&gt;.&lt;pubkey&gt;.&lt;msg&gt;</code>. See spec below.</p>

  <h2>Endpoints</h2>
  <table>
    <tr><th style="width:auto">Method</th><th>Path</th><th>Function</th></tr>
    <tr><td><code>GET</code></td><td><a href="/info">/info</a></td><td>gateway metadata (JSON)</td></tr>
    <tr><td><code>GET</code></td><td><a href="/health">/health</a></td><td>load + RPS + nonce count</td></tr>
    <tr><td><code>GET</code></td><td><a href="/stats/recent">/stats/recent</a></td><td>recent payments + cumulative totals</td></tr>
    <tr><td><code>GET</code></td><td><a href="/stats/leaderboard">/stats/leaderboard</a></td><td>top paying pubkeys</td></tr>
    <tr><td><code>GET</code></td><td><a href="/stats/qos">/stats/qos</a></td><td>QoS dispatcher state</td></tr>
    <tr><td><code>POST</code></td><td><code>/rpc</code></td><td>gated JSON-RPC proxy</td></tr>
    <tr><td><code>POST</code></td><td><code>/escrow/deposit</code></td><td>credit escrow on-chain</td></tr>
    <tr><td><code>GET</code></td><td><code>/escrow/balance/:pubkey</code></td><td>query escrow balance</td></tr>
    <tr><td><code>GET</code></td><td><code>/reputation/:pubkey</code></td><td>Trust-Score lookup</td></tr>
  </table>

  <h2>Interactive (UI)</h2>
  <ul>
    <li><a href="https://app.rpcpriority.com/try" target="_blank">app.rpcpriority.com/try</a> — fire a 402 challenge from the browser</li>
    <li><a href="https://app.rpcpriority.com/live" target="_blank">app.rpcpriority.com/live</a> — load + leaderboard + audit stream</li>
    <li><a href="https://app.rpcpriority.com/explorer" target="_blank">app.rpcpriority.com/explorer</a> — Trust-Score lookup by pubkey</li>
  </ul>

  <h2>Specification</h2>
  <ul>
    <li><a href="https://github.com/flavioparah/x402-priority-protocol/blob/main/docs/rfc/x402-priority.md" target="_blank">x402-priority v1.0</a> — wire protocol</li>
    <li><a href="https://github.com/flavioparah/x402-priority-protocol/blob/main/docs/rfc/x402-trust-score.md" target="_blank">x402-trust-score v0.1</a> — reputation broker</li>
    <li><a href="https://github.com/flavioparah/x402-priority-protocol/blob/main/docs/rfc/x402-qos-cooperative.md" target="_blank">x402-qos-cooperative v1.0</a> — operator-side QoS hint</li>
  </ul>

  <div class="footer">
    <a href="https://rpcpriority.com">rpcpriority.com</a>
    · <a href="mailto:flavio@rpcpriority.com">flavio@rpcpriority.com</a>
    · Built on <a href="https://x402.org" target="_blank">x402</a> (Coinbase) + Solana
  </div>
</div>
</body>
</html>`);
