...
let _incAdminAction = null;
function getIncAdminAction() {
  if (!_incAdminAction) {
    try { _incAdminAction = require("./metrics").incAdminAction; }
    catch { _incAdminAction = () => {}; }
  }
  return _incAdminAction;
}
...
function makeAdminGuards({ store, config }) {
  async function auditAdminWrite(req, action, target, outcome, extra = {}) {
    ...
    if (outcome === "ok" && action) {
      try { getIncAdminAction()(action, "ok"); } catch {}
    }
    return entry;
  }
