...
const { computeScoreV02 } = require("./trust-score");
...
async function buildAgentStatus(store, pubkey, opts) {
  ...
  const [rec, abuseHist, ban, isPerm, fraud, attestations] = await Promise.all([
    ...
    store.getAttestations(pubkey, 100).catch(() => []),
  ]);

  // ── Trust-Score / band / multiplier ──────────────────────────────────────
  const trust_score      = computeScoreV02(rec, attestations, { h1Active: false });
  const trust_multiplier = getTrustMultiplier(trust_score);
  const trust_band       = getTrustBand(trust_score);

  // ── Abuse-history counters ────────────────────────────────────────────────
  const throttles_5m  = abuseHist.filter(h => h.tier === 1 && now - h.ts <= FIVE_MIN_MS  ).length;
  const soft_bans_24h = abuseHist.filter(h => h.tier === 2 && now - h.ts <= ONE_DAY_MS   ).length;
  const hard_bans_7d  = abuseHist.filter(h => h.tier === 3 && now - h.ts <= SEVEN_DAYS_MS).length;
