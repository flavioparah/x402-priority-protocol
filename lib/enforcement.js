...
async function recordOffense(store, key, reason, opts) {
  ...
  if (fraudSignals.length > 0 && summary.throttles_5m >= 1) {
    ...
    escalated = true;
    await store.setBan(key, 3, reason, T.HARD_BAN_DURATION_MS);
  ...
  // ── Step 2: 3 throttles in 5min OR 10 invalid-sigs in 60s → tier 2 ────
    ...
    if (requireFraudCorroboration && fraudSignals.length === 0) {
      ...
      escalated = true;
      await store.setBan(key, 2, reason, T.SOFT_BAN_DURATION_MS);
 ...
 * @param {{tier:number, reason:string, until:number|null, limit?:number,
 *          windowSeconds?:number, remaining?:number, yourScore?:number,
 *          historySummary?:object, nextTierAt?:string}} state
