...
async function noncePreCheck(parts, store) {
  ...
  const traced = _wrapForTrace(payload);
  const nonce = traced?.nonce;
  ...
  return { ok: true, nonce, nonceData, messageBytes, payload: traced };
}
...
module.exports = {
  ...
  NONCE_RE,
  MAX_MESSAGE_BYTES,
  __resetTrace: () => { _accessed.length = 0; },
  __getTrace: () => [..._accessed],
};
