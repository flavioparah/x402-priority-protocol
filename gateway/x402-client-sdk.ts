...
export interface X402ProviderConfig extends ConnectionConfig {
  /** Max budget in micro-lamports per request */
  priorityBudget?: number;

  /**
   * Settlement mode. Currently only "offchain" is supported.
   */
  settlementMode?: "offchain";

  /** Callback before paying — return false to cancel */
  onChallenge?: (challenge: X402Challenge) => boolean | Promise<boolean>;

  /** Telemetry callback */
  onPayment?: (info: { nonce: string; amount: number; pubkey: string }) => void;

  /** Explicitly include commitment for type safety */
  commitment?: any;
}
...
export class X402Provider extends Connection {
  ...
  private async _x402RpcRequest(methodName: string, args: unknown[]): Promise<unknown> {
    ...
    const bodyHash = await this._sha256({
      jsonrpc: "2.0",
    ...
    return this._fetchWithX402(this.endpoint, body, bodyHash);
  }
  ...
  async request<T = unknown>(method: string, params: unknown[]): Promise<T> {
    ...
    const bodyHash = await this._sha256(payload);
    const body = JSON.stringify(payload);
    return this._fetchWithX402<T>(this.endpoint, body, bodyHash);
  }

  // ─── Lógica central de negociação ──────────────────────────────────────────

  private async _fetchWithX402<T>(url: string, body: string, bodyHash: string, retrying = false): Promise<T> {
    ...
    // Pay priority fee with body hash binding
    const authHeader = await this._payPriority(challenge, bodyHash);
   ...
   */
  private async _payPriority(challenge: X402Challenge, bodyHash: string): Promise<string> {
    if ((this.config.settlementMode as string) === "onchain") {
      return this._payPriorityOnChain(challenge);
    }
    return this._payPriorityOffChain(challenge, bodyHash);
  }

  private _payPriorityOffChain(challenge: X402Challenge, bodyHash: string): string {
    const pubkey = this.keypair.publicKey.toBase58();
    const payload = JSON.stringify({
      protocol: "x402-shield",
      network: this.endpoint.includes("devnet") ? "devnet" : "mainnet",
      nonce: challenge.nonce,
      pubkey,
      amount: challenge.amount_micro_lamports,
  ...
  /** Helper for SHA-256 hashing using canonical JSON */
  private async _sha256(message: any): Promise<string> {
