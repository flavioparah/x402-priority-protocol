...
function createInMemoryStore() {
  ...
  const usedSigs = new Map();         // tx_signature → expiresAt
  ...
  return {
    ...
    // In-memory version uses a Map of sig -> expiresAt to allow pruning.
    async hasSignature(sig) {
      const expiresAt = usedSigs.get(sig);
    ...
    async addSignature(sig, ttlMs = 86400000) { // Default 24h
      usedSigs.set(sig, Date.now() + ttlMs);
    },

    // Atomic: validate nonce + amount + hintedPubkey + balance, then mark
    // nonce used and debit escrow.
...
function createRedisStore(url) {
  ...
  // mark nonce used (preserving TTL), and debit escrow in one
  ...
  return {
    ...
    // Deposit signatures (anti-double-spend)
    // Individual keys per signature with TTL for better Redis performance/memory.
    async hasSignature(sig) {
      return (await r.exists(`x402:sig:${sig}`)) === 1;
    },
    async addSignature(sig, ttlMs = 86400000) {
      const ttlSec = Math.ceil(ttlMs / 1000);
      await r.set(`x402:sig:${sig}`, "1", "EX", ttlSec, "NX");
    },
